...
After receiving a request URI of the form `openid4vp://authorize?...` from the verifier (with the local sample server, via `POST /request` or `POST /request-object`), call `PresentCredential`:
...
**Parameters**:
- `uriString`: The request URI (`openid4vp://authorize?...`)
