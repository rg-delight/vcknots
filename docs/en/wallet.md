...
The zero value of every type is the conforming behavior. Nothing is read from the environment. A profile that forbids a departure refuses it rather than ignoring it: HAIP refuses `Transport` wherever it is carried and every non-zero `Presenter`, on every presenter entry point (OpenID4VP 1.0, Digital Credentials API, Draft 24 and re-admission), before any network access (the Status List checker with `statuslist.ErrStatusListInsecureTransportForbidden`, the others with an invalid-input or `invalid_request` error), and `Hooks` need a draft profile.
