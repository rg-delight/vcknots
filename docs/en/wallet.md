...
Every check on the authorization response reports its own condition, so a wallet can tell the holder what went wrong rather than that the issuance failed: `ErrAuthorizationRedirectURIMismatch`, `ErrAuthorizationStateMismatch`, `ErrAuthorizationIssMismatch`, `ErrAuthorizationIssMissing`, `ErrAuthorizationCodeMissing` and `ErrAuthorizationRedirectInvalid` are matched with `errors.Is`, and an `error=` redirect arrives as an `*AuthorizationResponseError` carrying the error code, its description and the echoed state. `OID4VCIFinalAuthorization.RequestURIExpired(now)` reports the RFC 9126 §2.2 `request_uri` lifetime; the library applies it at the authorization endpoint (`ErrAuthorizationRequestURIExpired`) and leaves it to the caller to decide how long a started issuance stays resumable, because the holder's login at the issuer may well outlast the `request_uri` the browser already spent.

A refusal by the issuer or the authorization server names the endpoint it came from: the metadata, PAR, token and nonce requests report an `oid4vci.EndpointError` with a `Stage` (`StageIssuerMetadata`, `StageAuthorizationServerMetadata`, `StagePAR`, `StageToken`, `StageNonce`), the HTTP status and the RFC 6749 `error` code, and keep no part of the response body. The Credential and Deferred Credential endpoints keep reporting `types.CredentialEndpointError`, which carries the §8.3.1.2 error code instead. A Token Response the wallet cannot present is `ErrTokenTypeUnsupported`, and under HAIP a non-DPoP `token_type` is `oid4vci.ErrDPoPRequired`.
...
* `RequireExpiry` rejects a Request Object without `exp`; `MaxAge` bounds its lifetime (`exp - iat`, or `exp - now` when `iat` is absent). `RequireExpiry` is off in every profile (neither OpenID4VP 1.0 nor HAIP requires `exp` on a Request Object, and the official conformance verifiers sign without it); the HAIP profile applies a 10-minute `MaxAge` to Request Objects that do carry `exp` when the caller leaves it zero.
...
#### Parsing a Request Object the caller already holds

`Oid4vpPresenter.ParseRequestObject(requestObject, expectedClientID)` authenticates a Request Object supplied by value: a JWT the application fetched from `request_uri` itself, or one that reached it over a transport this library does not drive. It shares the builder with `ParsePresentationRequest`, so the same JWT yields the same `CredentialPresentationRequest` and the same typed errors either way, and every Authorization Request parameter is taken from the signed claims. `ParseDraft24RequestObject` is the Presentation Exchange counterpart of `ParseDraft24PresentationRequest`. The dispatcher exposes both as `ParseRequestObject` and `ParseDraft24RequestObject`.

`expectedClientID` is the `client_id` of the Authorization Request the Request Object belongs to; a mismatch is reported with `ErrRequestObjectClientIDMismatch`. Pass `""` only when the caller holds the Request Object alone: the `client_id` claim is still authenticated against the signing certificate, and only the comparison against an outer parameter that does not exist is skipped.

This entry point receives no `request_uri_method` and echoes no `wallet_nonce`, because the fetch happened outside the call; use `DeliveredByReference` to record a fetch that did happen so HAIP §5.1 is still met. A Digital Credentials API invocation is not a Request Object delivery and is parsed by `ParseDCAPIRequest`.
...
* **HAIP bounds Request Object lifetime.** A HAIP Request Object that carries `exp` is limited to a 10-minute lifetime by default (`MaxAge`); `exp` itself stays optional unless `RequireExpiry` is set. Final defaults are unchanged.
