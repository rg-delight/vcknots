...
Verifier から `openid4vp://authorize?...` 形式のリクエスト URI（ローカルのサンプルサーバーでは `POST /request` または `POST /request-object` で取得）を受け取ったら、`PresentCredential` を呼び出します。
...
**パラメータ**:
- `uriString`: リクエスト URI（`openid4vp://authorize?...`）
