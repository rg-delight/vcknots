...
| `request_uri` の GET / POST と `wallet_nonce` | `ParsePresentationRequest`、`Draft24().ParsePresentationRequest` | `request_uri` はライブラリ自身が取得します。POST では毎回新しい `wallet_nonce` と、`Oid4vpPresenter.WalletMetadata` があれば `wallet_metadata` を送ります。`RequestObjectValidationOptions.RequestURIPolicy` で、取得の前に `request_uri` と `client_id` の関連を確かめられます。 |
...
この変数は、既定の receiver を構築するとき（`NewWallet`、`receiver.WithDefaultConfig`）に読み取られます。
自分で構築した receiver は、この変数ではなく自身の `AllowHTTP` フィールドに従います。
OpenID4VP の presenter はこの変数を読みません。
平文 HTTP は OpenID4VP の外にあるので、presenter が受け入れるのは `SetExperimentalOptions(oid4vp.ExperimentalOptions{AllowHTTP: true})` を呼んだときだけです（下のサンプルのとおりです）。
テストコードからは `env.SetHTTPAllowed(true)`（パッケージ `github.com/trustknots/vcknots/wallet/env`）で設定できます。

> ⚠️ **セキュリティ警告**: 本番環境では `VCKNOTS_WALLET_HTTP_ALLOWED`、`AllowHTTP`、`oid4vp.ExperimentalOptions` を有効化しないでください。HAIP は HTTP と X.509 の緩和を拒否します。
...
`Config.TestHooks.PresentationExchangeResponse` はテストのために応答を書き換えます。
...
* **`request_uri`**: POST では毎回新しい `wallet_nonce` を送り、Request Object はそれを echo しなければなりません。`WalletMetadata` があればそれも送ります（§5.11）。`request_uri_method` は大文字と小文字を区別します。
...
receiver の `AllowHTTP` と、presenter の実験用の `AllowHTTP` と `InsecureSkipX509Verify` は拒否します。
...
| 変数 | 既定値 | 説明 |
| :---- | :---- | :---- |
| `VCKNOTS_WALLET_HTTP_ALLOWED` | `false`（未設定 / 空） | `true` のとき、既定の receiver が HTTP エンドポイントを許可します（ローカル開発のみ）。構築時に読み取られます。client assertion を平文 HTTP で送るのは loopback ホストに対してだけです。OpenID4VP の presenter は読みません。HAIP は HTTP を拒否します。 |
...
* `oid4vp.Oid4vpPresenter` は `==` で比較できなくなりました。`oid4vci.Oid4vciReceiver` と `oid4vp.Oid4vpPresenter` には新しいフィールド（`HTTPClient`、`Profile` など。receiver には `AllowHTTP` も）とメソッドがあります。ゼロ値は `VCKNOTS_WALLET_HTTP_ALLOWED` の値にかかわらず HTTPS を要求します。`receiver.WithDefaultConfig` と `NewWallet` は receiver を構築するときにこの変数を読み、presenter はどれも読みません。
...
4. **HTTPS の強制:**
    - wallet はデフォルトで Issuer と Verifier のエンドポイントに HTTPS を要求します。Issuer への `VCKNOTS_WALLET_HTTP_ALLOWED=true` や receiver の `AllowHTTP`、Verifier への `oid4vp.ExperimentalOptions.AllowHTTP` による緩和は、ローカル開発に限ってください。
...
* **Q: 受領が `credential issuer must use https scheme` で失敗する。**
  * **A:** wallet はデフォルトで HTTPS を要求します。HTTP のローカルサンプルサーバーに対しては、wallet を構築する前に `VCKNOTS_WALLET_HTTP_ALLOWED=true` を設定するか、自分で構築する receiver に `AllowHTTP` を設定してください。presenter が HTTP の Verifier を受け入れるのは、`SetExperimentalOptions(oid4vp.ExperimentalOptions{AllowHTTP: true})` を呼んだときだけです。
