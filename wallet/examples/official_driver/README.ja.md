...
| `profile` | string | `"final"` | `profile.Profile` のテキスト形式（`profile.ParseProfile`）です。`"final"`、`"haip"`、またはそれらに Option を続けたもの（例: `"final;RequireDPoP;RequirePAR"`）を指定します。`Config.Profiles` の唯一の要素と、driver が構築する両 plugin のプロファイルにします。未知の値と draft プロファイルは拒否します。 |
