...
// identifier, with a CA that certifies its signing key. Its resolver enables
// every mechanism, including the Credential Issuer Metadata `jwks` the
// template carries, and makes no network request.
...
func TestStatusListKeysWebResolution(t *testing.T) {
	...
	setup := func(t *testing.T) (*testOrigin, *Resolver, Request, testKey) {
		...
		template := Request{
			...
			IssuerMetadataJWKS: keySet(metadata.public),
		}
	...
	t.Run("the Credential Issuer Metadata jwks is not a key source", func(t *testing.T) {
		t.Parallel()
...
func TestStatusListKeysDID(t *testing.T) {
	...
	setup := func(t *testing.T, format string, linked bool) (*Resolver, Request, statuslist.KeyRequest) {
		...
		template := Request{
			...
			// A matching metadata key used to bind the DID on its own.
	...
	t.Run("an unbound DID is DID-only trust even when the metadata names its key", func(t *testing.T) {
		t.Parallel()
