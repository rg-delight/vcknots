...
func TestNewPS384Verifier(t *testing.T) {
	if NewPS384Verifier() == nil {
		t.Fatal("NewPS384Verifier() should not return nil")
	}
}

func ps384TestProof(t *testing.T, key *rsa.PrivateKey, payload []byte) credential.CredentialProof {
	t.Helper()
	hash := sha512.Sum384(payload)
	signature, err := rsa.SignPSS(rand.Reader, key, crypto.SHA384, hash[:], &rsa.PSSOptions{SaltLength: rsa.PSSSaltLengthEqualsHash, Hash: crypto.SHA384})
	if err != nil {
		t.Fatalf("Failed to sign test data: %v", err)
	}
	return credential.CredentialProof{Algorithm: jose.PS384, Signature: signature, Payload: payload}
}

func TestPS384Verifier_Verify(t *testing.T) {
	verifier := NewPS384Verifier()
	privateKey, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		t.Fatalf("Failed to generate test key: %v", err)
	}
	publicKeyJWK := &jose.JSONWebKey{Key: &privateKey.PublicKey, KeyID: "test-key-1", Algorithm: string(jose.PS384), Use: "sig"}
	payload := []byte("test message for signature")
	proof := ps384TestProof(t, privateKey, payload)
