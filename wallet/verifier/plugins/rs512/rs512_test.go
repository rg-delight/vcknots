...
func TestNewRS512Verifier(t *testing.T) {
	if NewRS512Verifier() == nil {
		t.Fatal("NewRS512Verifier() should not return nil")
	}
}

func rs512TestProof(t *testing.T, key *rsa.PrivateKey, payload []byte) credential.CredentialProof {
	t.Helper()
	...
	return credential.CredentialProof{Algorithm: jose.RS512, Signature: signature, Payload: payload}
}

func TestRS512Verifier_Verify(t *testing.T) {
	verifier := NewRS512Verifier()
	privateKey, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		t.Fatalf("Failed to generate test key: %v", err)
	}
	publicKeyJWK := &jose.JSONWebKey{Key: &privateKey.PublicKey, KeyID: "test-key-1", Algorithm: string(jose.RS512), Use: "sig"}
	payload := []byte("test message for signature")
	proof := rs512TestProof(t, privateKey, payload)
