...
func TestNewPS256Verifier(t *testing.T) {
	if NewPS256Verifier() == nil {
		t.Fatal("NewPS256Verifier() should not return nil")
	}
}

func ps256TestProof(t *testing.T, key *rsa.PrivateKey, payload []byte) credential.CredentialProof {
	t.Helper()
	hash := sha256.Sum256(payload)
	signature, err := rsa.SignPSS(rand.Reader, key, crypto.SHA256, hash[:], &rsa.PSSOptions{SaltLength: rsa.PSSSaltLengthEqualsHash, Hash: crypto.SHA256})
	if err != nil {
		t.Fatalf("Failed to sign test data: %v", err)
	}
	return credential.CredentialProof{Algorithm: jose.PS256, Signature: signature, Payload: payload}
}

func TestPS256Verifier_Verify(t *testing.T) {
	verifier := NewPS256Verifier()
	privateKey, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		t.Fatalf("Failed to generate test key: %v", err)
	}
	publicKeyJWK := &jose.JSONWebKey{Key: &privateKey.PublicKey, KeyID: "test-key-1", Algorithm: string(jose.PS256), Use: "sig"}
	payload := []byte("test message for signature")
	proof := ps256TestProof(t, privateKey, payload)
