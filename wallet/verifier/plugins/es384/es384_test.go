...
func TestNewES384Verifier(t *testing.T) {
	if NewES384Verifier() == nil {
		t.Fatal("NewES384Verifier() should not return nil")
	}
}

func es384TestProof(t *testing.T, key *ecdsa.PrivateKey, payload []byte) credential.CredentialProof {
	t.Helper()
	...
	r.FillBytes(signature[:96/2])
	s.FillBytes(signature[96/2:])
	return credential.CredentialProof{Algorithm: jose.ES384, Signature: signature, Payload: payload}
}

func TestES384Verifier_Verify(t *testing.T) {
	verifier := NewES384Verifier()
	privateKey, err := ecdsa.GenerateKey(elliptic.P384(), rand.Reader)
	if err != nil {
		t.Fatalf("Failed to generate test key: %v", err)
	}
	publicKeyJWK := &jose.JSONWebKey{Key: &privateKey.PublicKey, KeyID: "test-key-1", Algorithm: string(jose.ES384), Use: "sig"}
	payload := []byte("test message for signature")
	proof := es384TestProof(t, privateKey, payload)
