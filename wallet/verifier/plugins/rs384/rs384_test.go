...
func TestNewRS384Verifier(t *testing.T) {
	if NewRS384Verifier() == nil {
		t.Fatal("NewRS384Verifier() should not return nil")
	}
}

func rs384TestProof(t *testing.T, key *rsa.PrivateKey, payload []byte) credential.CredentialProof {
	t.Helper()
	hash := sha512.Sum384(payload)
	signature, err := rsa.SignPKCS1v15(rand.Reader, key, crypto.SHA384, hash[:])
	if err != nil {
		t.Fatalf("Failed to sign test data: %v", err)
	}
	return credential.CredentialProof{Algorithm: jose.RS384, Signature: signature, Payload: payload}
}

func TestRS384Verifier_Verify(t *testing.T) {
	verifier := NewRS384Verifier()
	privateKey, err := rsa.GenerateKey(rand.Reader, 2048)
	if err != nil {
		t.Fatalf("Failed to generate test key: %v", err)
	}
	publicKeyJWK := &jose.JSONWebKey{Key: &privateKey.PublicKey, KeyID: "test-key-1", Algorithm: string(jose.RS384), Use: "sig"}
	payload := []byte("test message for signature")
	proof := rs384TestProof(t, privateKey, payload)
