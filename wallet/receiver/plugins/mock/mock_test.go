...
func TestMockReceiver_FetchAccessToken(t *testing.T) {
	...
	// Test with correct receiving type
	token, err := receiver.FetchAccessToken(types.Mock, *endpoint, types.PreAuthorizedCodeTokenRequest{PreAuthorizedCode: "test_code"})
	require.NoError(t, err)
	...
	// Test with incorrect receiving type
	_, err = receiver.FetchAccessToken(types.Oid4vci, *endpoint, types.PreAuthorizedCodeTokenRequest{PreAuthorizedCode: "test_code"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "unsupported receiving type")
}
