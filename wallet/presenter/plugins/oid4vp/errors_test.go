...
func TestSubmitErrorResponseRefusesAnUnadmittedRequest(t *testing.T) {
	...
	defer server.Close()
	request := admitDirectPost(t, withExperimental(&Oid4vpPresenter{}, experimental.Presenter{Transport: experimental.Transport{AllowHTTP: true}}), server.URL+"/response", "")

	_, err := withExperimental(&Oid4vpPresenter{}, experimental.Presenter{Transport: experimental.Transport{AllowHTTP: true}}).SubmitErrorResponse(context.Background(), request, "access_denied", "")
