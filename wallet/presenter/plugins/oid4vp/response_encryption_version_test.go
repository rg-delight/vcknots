...
func TestResponseEncryptionFollowsTheRequestVersion(t *testing.T) {
	...
	request, err := p.ParseDraft24PresentationRequest(uri)
	require.NoError(t, err, "Draft24 is admitted without the HAIP P-256 rule")

	_, err = p.CreateEncryptedAuthorizationResponse(map[string]any{"vp_token": "vp"}, request.ClientMetadata)
	...
	_, err = p.CreateEncryptedAuthorizationResponse(map[string]any{"vp_token": "vp"}, &VerifierMetadata{Jwks: jose.JSONWebKeySet{Keys: []jose.JSONWebKey{{
		Key: &key.PublicKey, KeyID: "p384", Use: "enc", Algorithm: "ECDH-ES",
	}}}})
