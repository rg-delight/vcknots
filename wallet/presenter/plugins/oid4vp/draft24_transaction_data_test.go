...
func TestDraft24TransactionDataValidation(t *testing.T) {
	...
	// Draft 24 DCQL is refused before transaction_data is read (decision
	// D1): this library answers dcql_query as OpenID4VP 1.0 only.
	_, err = parse(t, map[string]string{"dcql_query": `{"credentials":[{"id":"pid","format":"vc+sd-jwt"}]}`}, encode(`{"type":"example","credential_ids":["pid"]}`), []string{"example"})
